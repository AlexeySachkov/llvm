...
int main() {
  ...
  assert((one_dim.get_group_range() == sycl::id<1>{2}));
  assert(one_dim.get_group_range(0) == 2);
  assert((one_dim.get_global_range() == sycl::range<1>{4}));
  assert((one_dim.get_global_range(0) == 4));
  assert((one_dim.get_local_range() == sycl::range<1>{2}));
  assert((one_dim.get_local_range(0) == 2));
  assert((one_dim.get_offset() == sycl::id<1>{0}));
  assert((one_dim.get_nd_range() == sycl::nd_range<1>({4}, {2}, {0})));

  // two dimension nd_item
  sycl::nd_item<2> two_dim = Builder::createNDItem<2>(
      Builder::createItem<2, true>({4, 4}, {2, 2}, {0, 0}),
      Builder::createItem<2, false>({2, 2}, {0, 0}),
      Builder::createGroup<2>({4, 4}, {2, 2}, {1, 1}));
  assert((two_dim.get_global_id() == sycl::id<2>{2, 2}));
  assert(two_dim.get_global_id(0) == 2);
  assert(two_dim.get_global_id(1) == 2);
  assert(two_dim.get_global_linear_id() == 10);
  assert((two_dim.get_local_id() == sycl::id<2>{0, 0}));
  ...
  assert((three_dim.get_local_id() == sycl::id<3>{0, 0, 0}));
