// RUN: %clangxx -fsycl -fsycl-targets=%sycl_triple %s -Wno-deprecated-declarations -o %t.out
// RUN: %t.out
...
int main() {
  ...
  sycl::item<1, true> one_dim_with_offset =
      Builder::createItem<1, true>({4}, {2}, {1});
  assert(one_dim_with_offset.get_id() == sycl::id<1>{2});
  assert(one_dim_with_offset.get_id(0) == 2);
  assert(one_dim_with_offset[0] == 2);
  assert(one_dim_with_offset.get_range() == sycl::range<1>{4});
  assert(one_dim_with_offset.get_range(0) == 4);
  assert(one_dim_with_offset.get_offset() == sycl::id<1>{1});
  assert(one_dim_with_offset.get_linear_id() == 1);

  // two dimension item with offset
  sycl::item<2, true> two_dim_with_offset =
      Builder::createItem<2, true>({4, 8}, {2, 4}, {1, 1});
  assert((two_dim_with_offset.get_id() == sycl::id<2>{2, 4}));
  ...
  assert((three_dim_with_offset.get_range() == sycl::range<3>{4, 8, 16}));
  ...
  assert((three_dim_with_offset.get_offset() == sycl::id<3>{1, 1, 1}));
  assert(three_dim_with_offset.get_linear_id() == 183);

  // one dimension item without offset
  sycl::item<1, false> one_dim = Builder::createItem<1, false>({4}, {2});
  assert(one_dim.get_id() == sycl::id<1>{2});
  assert(one_dim.get_id(0) == 2);
  assert(one_dim[0] == 2);
  assert(one_dim.get_range() == sycl::range<1>{4});
  assert(one_dim.get_range(0) == 4);
  assert(one_dim.get_linear_id() == 2);

  // two dimension item without offset
  sycl::item<2, false> two_dim = Builder::createItem<2, false>({4, 8}, {2, 4});
  assert((two_dim.get_id() == sycl::id<2>{2, 4}));
  assert(two_dim.get_id(0) == 2);
  assert(two_dim.get_id(1) == 4);
  assert(two_dim[0] == 2);
  assert(two_dim[1] == 4);
  assert((two_dim.get_range() == sycl::range<2>{4, 8}));
  ...
  assert((three_dim.get_id() == sycl::id<3>{2, 4, 8}));
  ...
  assert((three_dim.get_range() == sycl::range<3>{4, 8, 16}));
  ...
  sycl::item<1, true> one_dim_transformed = one_dim;
  sycl::item<1, true> one_dim_check =
      Builder::createItem<1, true>({4}, {2}, {0});
  assert(one_dim_transformed == one_dim_check);
  sycl::item<2, true> two_dim_transformed = two_dim;
  sycl::item<2, true> two_dim_check =
      Builder::createItem<2, true>({4, 8}, {2, 4}, {0, 0});
  assert(two_dim_transformed == two_dim_check);
  sycl::item<3, true> three_dim_transformed = three_dim;
  sycl::item<3, true> three_dim_check =
      Builder::createItem<3, true>({4, 8, 16}, {2, 4, 8}, {0, 0, 0});
  assert((three_dim_transformed == three_dim_check));

  using value_type = decltype(std::declval<sycl::item<1>>()[0]);
